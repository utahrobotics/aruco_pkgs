...
class ArucoPatchNode(object):
    ...
    def __init__(self):
        ...
        self.rot_quat = (0, pitch, yaw) # for some reason this is 0
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(len(self.marker_id_list)):
                ...
                if self.tf_buffer._frameExists(frame_name): 
                    lookup_time = rospy.Time.now() - rospy.Duration(1.0)
                    if self.tf_buffer.can_transform('map', frame_name, rospy.Time()):
                        geomsg_tfstamped = self.tf_buffer.lookup_transform('map', \
                                frame_name, rospy.Time())
            ...
            if len(live_camera_tfs) > 1:
                ...
                out_tf = odom_tf.dot(inversed_transform)
